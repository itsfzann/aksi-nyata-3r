...
const quizSlideIndex = [...document.querySelectorAll(".slide")].findIndex(
...
slides.forEach((slide, i) => {
  ...
  item.textContent = `${String(i + 1).padStart(2, "0")}  ${titles[i]}`;
...
function showSlide(index) {
  ...
  prevBtn.disabled =
    currentSlide === 0 || (currentSlide === quizSlideIndex && !quizCompleted);
...
function nextSlide() {
  if (currentSlide === quizSlideIndex && !quizCompleted) {
    return;
  }
...
const rData = {
  reduce: {
    ...
    text: "Mengurangi penggunaan barang yang berpotensi menjadi sampah, terutama barang sekali pakai.",
  ...
  reuse: {
    ...
    text: "Menggunakan kembali barang yang masih layak agar masa pakainya lebih panjang dan tidak cepat menjadi sampah.",
  ...
  recycle: {
    ...
    text: "Mengolah sampah menjadi bahan atau produk baru yang dapat dimanfaatkan kembali.",
...
document.querySelectorAll(".learn-btn").forEach((button) => {
  button.addEventListener("click", () => {
    const data = rData[button.dataset.r];
    ...
    modalExamples.innerHTML = data.examples
      .map((example) => `<div class="modal-example">✓ ${example}</div>`)
      .join("");
...
function loadQuiz() {
  ...
  quizProgress.textContent = `Soal ${qIndex + 1} / ${questions.length}`;

  quizScore.textContent = `Skor: ${score} / ${questions.length * pointsPerQuestion}`;
...
function answerQuiz(selected) {
  ...
  if (selected === correct) {
    ...
    feedbackEl.style.color = "#238b57";
  } else {
    buttons[selected].classList.add("wrong");

    feedbackEl.textContent = `✕ Belum tepat. Jawaban yang benar: ${questions[qIndex].a[correct]}`;

    feedbackEl.style.color = "#a74d43";
  }
...
function showQuizResult() {
  const percent = (score / (questions.length * pointsPerQuestion)) * 100;
  ...
  questionEl.textContent = `${title} Kamu mendapat ${score} dari ${questions.length * pointsPerQuestion} poin.`;
  ...
  nextQuestion.textContent = "Ulangi Quiz";
